...
function slideFn(selEl) { // selEl 선택 슬라이드 요소
    ...
    const TIME_SLIDE = 400;
    // 1. 대상선정
    // 1-1. 슬라이드 부모요소: 전달된 요소 .slide
    const sldWrap = selEl;
    // 1-2.변경 대상: 전달된 선택요소 -> selEl
    const slide = dFn.qs(sldWrap,'.slide');
    // 1-1.이벤트 대상: 선택슬라이드 하위 .abtn
    const abtn = dFn.qsaEl(sldWrap, '.abtn');
    // 1-3.블릿박스 대상:
    const indic = dFn.qsaEl(sldWrap, '.indic');
    ...
    // 대상: .incid -> indic 변수
    let sldCnt = dFn.qsaEl(slide, 'li').legnth;
